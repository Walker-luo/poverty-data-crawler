...
| 日期字段 | 绝对日期 (如 `2025-12-10`) | 相对时间 (如 `5 年`) + URL提取 `pub_date` |
| 可用性 | ⚠️ 大概率不可用 | ✅ 稳定可用 |
| 推荐 | 不推荐 | **推荐** |
...
Bing 不需要 site 搜索，简化为两阶段：

```
Phase 1 (主力): 15 关键词 × N 年逐年搜索
        → 每个关键词+年份独立搜索，结果不重叠
        → 自动跳过连续 3 年无结果的冷门关键词
...
| `title` | 文章标题 (已去 HTML) | ✅ | ✅ |
| `url` | 原文链接 | ✅ | ✅ |
| `date` | 搜索引擎显示的发布时间 | `"2025-12-10"` | `"5 年"` (相对) |
| `pub_date` | 从URL提取的绝对日期 | — | `"2021-02-26"` |
| `summary` | 文章摘要 (前200字) | ✅ | ✅ |
| `source` | 来源媒体名称 | ✅ | ✅ |
| `is_official` | 是否官方媒体 | ✅ | ✅ |
| `search_mode` | 采集阶段标记 | `year_partition/keyword/site` | `year/catchup` |
| `crawl_time` | 数据采集时间 | ✅ | ✅ |
...
```
data/processed/news/20260731_211505/
├── news.csv          ← 结构化数据
...
├── utils/
│   ├── data_processor.py     # 数据清洗/去重/日期标准化
│   └── storage.py            # 存储 + Markdown 统计报告生成
├── data/
